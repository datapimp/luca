...
namespace :site do
  desc "Publishes the documentation site"
  task :publish do
    `git co master downloads`
    `cp downloads/luca*.js app/assets/javascripts/vendor`
    `cd site && bundle exec middleman build && cd ..`
    `cp -r site/build/* .`
    `git add . && git commit -m "publishing site"`
  end
end
