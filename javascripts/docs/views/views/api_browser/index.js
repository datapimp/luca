...
<html class='no-js'>
  ...
  view.privateMethods({
    runExample: function() {
      this.findComponentByName("endpoint_url", true).setValue("https://api.github.com/users/datapimp/gists");
      return this.loadUrl();
    },
    loadUrl: function() {
      var url,
        _this = this;
      url = this.findComponentByName("endpoint_url", true).getValue();
      return $.get(url, function(parsed, state, options) {
        _this.getOutput().$html(options.responseText);
