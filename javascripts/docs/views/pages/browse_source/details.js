...
<html class='no-js'>
  ...
  view.contains({
    ...
    loadComponent: function(model) {
      this.$el.empty();
      this.$el.append("<h2>" + (model.get('class_name')) + "</h2>");
      return this.$el.append("<div class='header-documentation'>" + (model.get('header_documentation')) + "</div>");
    }
  }, {
  ...
  view.defines({
    afterRender: function() {
      this.getDetails().$el.hide();
      return this.getDocumentation().$el.hide();
    },
    load: function(model) {
      this.getDetails().$el.show();
      this.getDocumentation().$el.show();
      this.getDetails().loadComponent(model);
      this.getDocumentation().loadComponent(model);
      return this.prettyPrint();
    },
    prettyPrint: function() {
      this.$('pre').addClass('prettyprint');
      return typeof window.prettyPrint === "function" ? window.prettyPrint() : void 0;
    }
  });
